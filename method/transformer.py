...
from .attn2 import MultiheadAttention
...
class ResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        self.attn = MultiheadAttention(d_model, n_head, dropout=n_dropout)
    ...
    def attention(
        ...
        return self.attn(
            q_x, k_x, v_x, need_weights=True, attn_mask=attn_mask, diag_attn=diag_attn
        )
    ...
    def forward(
        ...
        tmp, attn = self.attention(q_x=self.ln_1(q_x), k_x=k_x, v_x=v_x, attn_mask=attn_mask, diag_attn=diag_attn)
...
class Transformer(nn.Module):
    ...
    # def custom_attn(self, attn_layer, x, attn_mask=None, attn_type="fused-attn"):
    ...
    #         attn_weights = F.softmax(attn_weights, dim=-1)
    #     elif attn_type == "q-q&k-k&v-v":
    #         qq_attn_weights = torch.bmm(q * scale, q.transpose(1, 2))
    #         qq_attn_weights = F.softmax(qq_attn_weights, dim=-1)
    #         kk_attn_weights = torch.bmm(k * scale, k.transpose(1, 2))
    #         kk_attn_weights = F.softmax(kk_attn_weights, dim=-1)
    #         vv_attn_weights = torch.bmm(v * scale, v.transpose(1, 2))
    #         vv_attn_weights = F.softmax(vv_attn_weights, dim=-1)
    #         attn_weights    = (qq_attn_weights + kk_attn_weights + vv_attn_weights) / 3
    ...
    def forward(self, x: torch.Tensor, out_layers: list = [3, 6, 9],
                attn_mask: Optional[torch.Tensor] = None, visual: bool = False):
        if not visual:
            ...
            for r in self.resblocks:
                ...
                if self.grad_checkpointing and not torch.jit.is_scripting():
                    ...
                    if idx in out_layers:
                        x_finegrain , _ = self.custom_attn(r.attn, r.ln_1(x), attn_mask=None, attn_type="v-v")
                        out_tokens.append(x_finegrain)
                        x_list.append(x)
                        x, attn_tmp = r(x, attn_mask=attn_mask)
                    else :
                        x, attn_tmp = r(x, attn_mask=attn_mask)
                    fusion_attn_map = fusion_attn_map + attn_tmp
            
            model_res = self.resblocks[-1]
            for x_for_fusion in x_list:
                ln_x, _ = self.custom_attn(model_res.attn, model_res.ln_1(x_for_fusion), attn_mask=fusion_attn_map, mid_simi=None, attn_type="fused-attn")
                out_tokens.append(ln_x)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, out_layers: list, single_output : bool = True):
        ...
        x, patch_tokens = self.transformer(x, out_layers, visual = True)
        ...
        if self.attn_pool is not None:
            ...
            for patch_token in patch_tokens:
                pooled, tokens = self._global_pool(patch_token)
        ...
        if self.proj is not None:
            img_feats = [pooled @ self.proj for pooled in img_feats]
            # patch_tokens = patch_tokens @ self.proj  
            # tokens = tokens @ self.proj

        if self.output_tokens:
            return img_feats, patch_tokens

        if single_output:
            return img_feats[-1], patch_tokens
        else : 
            return img_feats, patch_tokens
